...
export const ScatterTeachersDistribution = memo(function ScatterTeachersDistribution({
  modelId,
  newTeacherState,
}: PropsType) {
  ...
  const renderData = () => {
    ...
    return (
      <>
        <Scatter ref={chartRef} data={data} options={options} width={400} height={400} />
      </>
    );
  };
  ...
  return <>{plotData.length ? renderData() : <CircularProgress />}</>;
});
